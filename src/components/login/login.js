...
import { getBiodata, getLogin } from '../../actions/userAction';
...
export const Login = () => {
  ...
  const {getBiodataResult} = useSelector(
    (state)=>state.UserReducers);
  ...
  useEffect(() => {
    ...
    else if (getLoginLoading) {
      
    }
    else if(getLoginError.data){
      Swal.fire({
        title: "Gagal Login!",
        text: `${getLoginError.data.message}`,
        icon: "error",
        showConfirmButton:false,
        timer:1000
      })
  ...
  }, [dispatch,getLoginError,getLoginLoading,getLoginResult])
